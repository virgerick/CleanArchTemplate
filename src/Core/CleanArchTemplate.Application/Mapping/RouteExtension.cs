...
public static class MappingExtensions
{
    ...
    {
        return new RouteResponse(route.Id.Value, route.Origin, route.Destination, route.Distance, route.EstimatedTime, route.Amount);

    }
    public static IQueryable<RouteResponse> ProjectTo(this IQueryable<Route> source,int index)
    {
        if(source==null) throw new ArgumentNullException(nameof(source));
        return source.Select(route=>new RouteResponse(route.Id.Value, route.Origin, route.Destination, route.Distance, route.EstimatedTime, route.Amount));
    }
}
