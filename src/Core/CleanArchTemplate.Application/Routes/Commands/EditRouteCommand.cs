...
public record struct EditRouteCommand(Guid Id, string Origin, string Destination, float Distance, float EstimatedTime, decimal Amount, Guid VehicleId) : IRequest<OneOf<Guid, Exception>>;
public sealed class EditRouteCommandHandler : IRequestHandler<EditRouteCommand, OneOf<Guid, Exception>>
{
    ...
    public async Task<OneOf<Guid, Exception>> Handle(EditRouteCommand request, CancellationToken cancellationToken)
    {
        ...
        found.Update(request.Origin, request.Destination, request.Distance, request.EstimatedTime, request.Amount, vehicleId)
        .Switch(
